...
// Fallback demo data while Supabase loads
const DEMO = [
    ...
    try {
      const res = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 500,
          system: `You are Eden's AI beauty concierge for UK salons near ${postcode}. Be warm and specific — mention salon names, services and prices. Available venues: ${JSON.stringify(salons.slice(0,6).map(s=>({name:s.name,cat:s.category,rating:s.rating,tags:s.tags,services:(s.services||[]).slice(0,4).map(sv=>({n:sv.name,p:sv.price}))})))}. Reply in 2–3 sentences max. End with one clear recommendation.`,
          messages: [{ role: 'user', content: aiQ }],
        }),
      })
      const d = await res.json()
      setAiA(d.content?.map(c=>c.text||'').join('') || "I couldn't find a match — try rephrasing.")
      ...
      <div style={{ display:'flex',height:'calc(100vh - 62px)' }}>
        ...
        <div style={{ width: selected ? 400 : '100%', maxWidth: selected ? 400 : 'none', overflowY:'auto', borderRight:`1px solid ${T.border}`, background:T.cream, flexShrink:0, transition:'width 0.3s ease' }}>
          ...
          <div style={{ padding:'12px 14px',background:T.mint,borderBottom:`1px solid ${T.sagePale}` }}>
            <div style={{ fontSize:9,letterSpacing:3,color:T.moss,fontWeight:700,marginBottom:7 }}>✦ EDEN AI CONCIERGE</div>
            <div style={{ display:'flex',gap:8 }}>
              <input value={aiQ} onChange={e=>setAiQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&askAI()}
                placeholder="'Best balayage under £150' or 'relaxing facial near me'…"
              ...
              <button onClick={askAI} disabled={aiLoading} style={{ padding:'9px 16px',background:T.forest,border:'none',borderRadius:8,color:T.white,fontSize:11,fontWeight:700,cursor:'pointer',display:'flex',alignItems:'center',gap:6,flexShrink:0 }}>
                {aiLoading ? <Spinner size={12} color={T.white}/> : 'Ask AI'}
              </button>
            </div>
          ...
          <div style={{ padding:'12px' }}>
            {loading ? (
              <div style={{ textAlign:'center',padding:'40px 0',color:T.inkSoft }}>
                <Spinner size={28}/><div style={{ marginTop:12,fontSize:13 }}>Finding salons near you…</div>
              </div>
            ) : sorted.length === 0 ? (
              <div style={{ textAlign:'center',padding:'40px 0',color:T.inkSoft }}>
              ...
              <SalonCard key={s.id} salon={s} selected={selected?.id===s.id}
                onSelect={setSelected} isFav={favs.includes(s.id)} onFav={() => setFavs(f=>f.includes(s.id)?f.filter(x=>x!==s.id):[...f,s.id])}
        ...
        {!selected && (
          <div style={{ flex:1,display:'flex',alignItems:'center',justifyContent:'center',background:T.offwhite }}>
            <div style={{ textAlign:'center',maxWidth:300 }}>
              ...
              <div style={{ fontSize:13,color:T.inkFaint,lineHeight:1.7 }}>Choose any salon to see details, services, prices and booking options.</div>
            </div>
          </div>
        )}
      </div>
